...
if (isset($_POST['tambah'])) {
    ...
    // Validate 'notlp' - must only contain numbers and have at least 12 characters
    if (!is_numeric($notlp)) {
    ...
    // If there are any validation errors, redirect back to form with error message
    if ($errorMessage != '') {
        echo "<script>
            alert('$errorMessage');
    ...
    if ($tambah && $query) {
        echo "<script>alert('Data kendaraan berhasil ditambahkan!'); window.location.href='kendaraan.php';</script>";
    } else {
        echo "<script>alert('Data kendaraan gagal ditambahkan!'); window.location.href='kendaraan.php';</script>";
    }
} else {
    echo "<script>alert('Request tidak valid!'); window.location.href='kendaraan.php';</script>";
}
?>
